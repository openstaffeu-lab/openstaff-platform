...
export class ProjectExecutionService {
  ...
  async checkIn(projectId: string, body: CheckInWorkerDto, user: AuthenticatedUser) {
    const context = await this.getExecutionContext(projectId, user, body.workerId, body.assignmentId);
    const assignment = context.assignment;

    if (!assignment || assignment.status !== ProjectWorkerAssignmentStatus.ACTIVE) {
      throw new ForbiddenException('Only active worker assignments can check in');
    }
    ...
    if (
      ...
      await this.registerInvalidExecutionSignals(assignment, eligibility, 'CHECK_IN_BLOCKED');
    ...
    if (existingOpen) {
      throw new BadRequestException('This worker already has an open check-in on the project');
    }
  ...
  async checkOut(projectId: string, body: CheckOutWorkerDto, user: AuthenticatedUser) {
    const context = await this.getExecutionContext(projectId, user, body.workerId, body.assignmentId);
    const assignment = context.assignment;

    if (!assignment || assignment.status === ProjectWorkerAssignmentStatus.BLOCKED) {
      throw new ForbiddenException('Blocked worker assignments cannot check out through this flow');
    }
  ...
  async createWorkLog(projectId: string, body: CreateWorkerWorkLogDto, user: AuthenticatedUser) {
    const context = await this.getExecutionContext(projectId, user, body.workerId, body.assignmentId);
    const assignment = context.assignment;

    if (!assignment) {
      throw new BadRequestException('A worker assignment is required for work logs in this phase');
    }
    ...
    if (
      ...
      throw new ForbiddenException('Work logs can only be created for approved or active assignments');
    }

    if (body.jobRequestId && assignment.jobRequestId && body.jobRequestId !== assignment.jobRequestId) {
      throw new BadRequestException('Work log job request does not match the assignment');
    }
  ...
  async updateWorkLogStatus(
    ...
    const access = await this.getProjectScopedAccess(projectId, user);
    const isReviewer = access.isOwner || (access.currentProfile?.profileType === ProfileType.SUPERVISOR);
    const isContractor = access.currentProfile?.id === workLog.worker.profileId;

    if (!isReviewer && !isContractor) {
      throw new ForbiddenException('You do not have access to update this work log');
    }
    ...
    if (
      ...
      if (!isReviewer) {
        throw new ForbiddenException('Only the project owner or supervisor can review work logs');
      }
    }

    if (body.status === WorkerWorkLogStatus.SUBMITTED || body.status === WorkerWorkLogStatus.DRAFT) {
      if (!isContractor && !isReviewer) {
        throw new ForbiddenException('Only the contractor side can submit or draft work logs');
      }
    }
    ...
    if (body.status === WorkerWorkLogStatus.APPROVED || body.status === WorkerWorkLogStatus.REJECTED) {
      await this.notificationService.createInAppNotification({
  ...
  private async reconcileMissingAttendanceTasks(projectId: string) {
    ...
    const activeAssignments = await this.prisma.projectWorkerAssignment.findMany({
      where: {
        projectId,
      ...
      if (!attendance) {
        await this.prisma.userTask.upsert({
          ...
          update: {
            ...
            description: 'No attendance record exists yet for this active worker assignment today.',
  ...
  private async registerInvalidExecutionSignals(
    ...
    await this.prisma.userTask.upsert({
      ...
      update: {
        ...
        description: reason || 'The worker is blocked from attendance due to compliance issues.',
  ...
  private async getProjectScopedAccess(projectId: string, user: AuthenticatedUser) {
    ...
    if (!linkedContract) {
      throw new ForbiddenException('You do not have access to this project execution workspace');
    }
    ...
    return {
      ...
      worker: attendance.worker
        ? {
            ...
            fullName: `${attendance.worker.firstName} ${attendance.worker.lastName}`.trim(),
