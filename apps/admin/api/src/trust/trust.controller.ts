...
export class TrustController {
  ...
  async requestEmailOwnershipVerification(@Req() req: any) {
    return buildSuccessResponse(
      await this.trustService.requestEmailOwnershipVerification(
        req.user.sub,
        req,
      ),
    );
  ...
  async confirmEmailOwnership(
    @Body() body: ConfirmTrustTokenDto,
    @Req() req: any,
  ) {
    return buildSuccessResponse(
...
export class TrustAdminController {
  ...
  async getTrustSummary(@Param('userId') userId: string) {
    try {
      return buildSuccessResponse(
        await this.trustService.getAdminTrustSummary(userId),
      );
    } catch (error) {
  ...
  async performTrustAction(
    ...
    try {
      return buildSuccessResponse(
        await this.trustService.performAdminTrustAction(
          userId,
          req.user.sub,
          body,
          req,
