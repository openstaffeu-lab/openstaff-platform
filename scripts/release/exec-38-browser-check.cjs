...
const targetUrl =
  process.env.EXEC38_TARGET_URL ||
  "https://backoffice.openstaff.eu/admin/production-readiness";
const accessToken = process.env.EXEC38_ACCESS_TOKEN;
const refreshToken = process.env.EXEC38_REFRESH_TOKEN;

if (!accessToken || !refreshToken) {
  throw new Error("EXEC38_ACCESS_TOKEN and EXEC38_REFRESH_TOKEN are required.");
}
...
async function runBrowser({ name, channel, contextOptions }) {
  ...
  try {
    ...
    const reasoningVisible =
      ...
      normalizedPageText.includes("source-of-truth");
    const staleVisibility =
      normalizedPageText.includes("freshness") ||
      normalizedPageText.includes("stale-state handling") ||
      normalizedPageText.includes("stale");
    const authorityBoundariesVisible =
      normalizedPageText.includes("advisory") &&
      normalizedPageText.includes("approve") &&
      normalizedPageText.includes("activate billing") &&
      normalizedPageText.includes("change rollout state");
