...
Write-Host "gcloud secrets create WEBHOOK_SECRET --project=$ProjectId --replication-policy=automatic"
...
Write-Host "echo 'REPLACE_WITH_SECURE_VALUE' | gcloud secrets versions add WEBHOOK_SECRET --project=$ProjectId --data-file=-"
...
Write-Host "gcloud secrets add-iam-policy-binding WEBHOOK_SECRET --project=$ProjectId --member=serviceAccount:CLOUD_RUN_SERVICE_ACCOUNT --role=roles/secretmanager.secretAccessor"
