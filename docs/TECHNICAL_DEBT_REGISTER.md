...
| medium | password recovery delivery gap | the secure reset flow now exists in application logic, but provider-backed delivery is still blocked by `emailDelivery = not_configured` | account recovery is safer than before, but still not fully closed as a production user journey | connect a real delivery provider before claiming a fully closed public recovery experience |
| medium | company lookup provider gap | onboarding now has a lookup abstraction and deterministic baseline matches, but not a fully live external registry integration | users get a better autofill experience, but not yet full registry-backed trust | connect live Romanian and EU provider coverage before removing manual fallback emphasis |
| medium | RELU suggestion application gap | RELU AI suggestions are now visible during onboarding completion, but applying them into structured profile fields still depends on manual profile editing | visible suggestions are useful, but the flow still has avoidable clerical steps | add structured accept or reject actions for RELU outputs in a follow-up execution |
